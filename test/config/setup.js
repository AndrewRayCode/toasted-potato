...
function validateAndSetUpAddons(addonsPath, callback) {
  if (!addonsPath) {
    error(colors.red.bold(`Please provide the absolute path to the Blender addons directory, as in:
  ...
  if (fs.existsSync(installTestDir)) {
    ...
    const currentAddonPath = path.resolve(__dirname, '..', '..', 'blender', 'io_three_development');
...
Is this OK? Y/n`);
    ...
    ${syminkCommand}`));
          ...
          error(colors.red.bold('You did not give permission to symlink the test directory. Cannot continue!'));
        }
      },
    );
  }
...
function validateAndGetBlenderPath(blederPathArg) {
  ...
  if (blederPathArg) {
    ...
    error(colors.bold
      .red(`Congrats! You are the first user to run this project on the platform "${colors.cyan(osPlatform)}"! Please make a pull request with the correct exectuable path of a default Blender install.
    Until then you run:
  ...
  if (!foundAndExecutable) {
    error(colors.red.bold(`I looked for Blender the file at this path: "${colors.cyan(blenderPath)}" but it does not exist or is not executable.
    Please verify this file exists, or pass in the path to the Blender executable via:
        ${colors.cyan('yarn test -- --blender-path=/path/to/blender/executable')}`));
  }
  return blenderPath;
}
module.exports = function setup(callback) {
  ...
  validateAndSetUpAddons(commandArgs['addons-path'], () => {
    const blenderPath = validateAndGetBlenderPath(commandArgs['blender-path']);
